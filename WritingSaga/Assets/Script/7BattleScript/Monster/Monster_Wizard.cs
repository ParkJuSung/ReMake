...
public class Monster_Wizard : Unit {
	...
	void Start () {
		...
		Current_State = Unit_State.Archer_Find;
		Wizard_Init();
	...
	void Update () {
		...
		DrawMonsterBar(HPBar, HeadUpPosition, HP, 300);
		DrawMonsterState(stateScreen, Current_State.ToString(), HeadUpPosition);

		slot5 = GameObject.Find("SkillSlot5");
		if(slot5 !=null)
			if (slot5.GetComponent<Skill>().Current_State == Skill.Skill_State.Casting_State)
			...
			switch (Current_State)
			{
				...
				case Unit_State.Fear_State:
					...
					if (FearEffect == null)
					{
						...
						Destroy(FearEffect, FearEffect.GetComponent<ParticleSystem>().duration);
					}
	...
	void FindTarget() // 적 탐색 함수
	{
		...
		if (SBattleManager.Instance.UnitList.Count > 0)
		{
			...
			for (int UnitIter = 0; UnitIter < SBattleManager.Instance.UnitList.Count; UnitIter++)
			{
				if (aliveArcher)
				{
					if (SBattleManager.Instance.UnitList[UnitIter].tag == "Archer")
					{
